...
void FSM::Calculate_CamBoneMatrix()
{
	...
	m_WeaponBoneMatrix = m_pOwner.lock()->Get_Animator()->Get_CurAnimTransform(m_iWeaponBoneIndex) *
		_float4x4::CreateRotationX(XMConvertToRadians(-90.f)) * _float4x4::CreateScale(0.01f) * _float4x4::CreateRotationY(XM_PI) * m_pOwner.lock()->GetOrAddTransform()->Get_WorldMatrix();
	...
	m_vWeaponBonePos = _float4{ m_WeaponBoneMatrix.Translation().x, m_WeaponBoneMatrix.Translation().y, m_WeaponBoneMatrix.Translation().z , 1.f };
}
