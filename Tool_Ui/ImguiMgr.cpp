...
void ImguiMgr::Decide_Transform_Value()
{
   ...
   if (ImGui::InputText("##Selected Texture Name", strName.data(), 256)) {
   }
   ...
   ImGui::SameLine();
   ImGui::SetNextItemWidth(200);
   ImGui::InputFloat("X##ScaleY1", &m_vecScale.y, 0.01f, 1.0f, "%.3f", eFlag);
...
void ImguiMgr::Change_Value_Object()
{
   ...
   ImGui::SameLine();
   ImGui::SetNextItemWidth(200);
   ImGui::InputFloat("##PosX2", &m_vecPos2.x, 0.01f, 1.0f, "%.3f", eFlag);

   ImGui::Text("Y");
   ImGui::SameLine();
   ImGui::SetNextItemWidth(200);
   ImGui::InputFloat("##PosY2", &m_vecPos2.y, 0.01f, 1.0f, "%.3f", eFlag);

   ImGui::Text("Z");
   ImGui::SameLine();
   ImGui::SetNextItemWidth(200);
   ImGui::InputFloat("##PosZ2", &m_vecPos2.z, 0.01f, 1.0f, "%.3f", eFlag);
   ...
   ImGui::SameLine();
   ImGui::SetNextItemWidth(200);
   ImGui::InputFloat("##ScaleX2", &m_vecScale2.x, 0.01f, 1.0f, "%.3f", eFlag);

   ImGui::Text("Y");
   ImGui::SameLine();
   ImGui::SetNextItemWidth(200);
   ImGui::InputFloat("##ScaleY2", &m_vecScale2.y, 0.01f, 1.0f, "%.3f", eFlag);
   ...
   if (ImGui::Button("Change##Pos_Scale", ImVec2(80.f, 20.f)) || m_bIsChangeCheck)
   {
      if (!ImGui::IsItemActive() || m_bIsChangeCheck)
      {
         ...
         transform->Set_State(Transform_State::POS, _float4(m_vecPos2.x, m_vecPos2.y, m_vecPos2.z, 1));
         transform->Scaled(_float3(m_vecScale2.x, m_vecScale2.y, 1.f));
      }
   }
...
{
   if (ImGui::Button("Open File Dialog##save load file"))
      ImGuiFileDialog::Instance()->OpenDialog("GetFile", "Get File", ".dat", ".");

   // display
   if (ImGuiFileDialog::Instance()->Display("GetFile"))
   {
      // action if OK
      if (ImGuiFileDialog::Instance()->IsOk())
      {
...
void ImguiMgr::Save_Ui_Desc()
{
   ...
   if (ImGui::Button("Save##Save button", ImVec2(60.f, 20.f)))
   {
      if (!ImGui::IsItemActive())
      {
         if (0 == m_strSaveLoadPath.length())
         {
            return;
         }

         shared_ptr<FileUtils> file = make_shared<FileUtils>();
         file->Open(m_strSaveLoadPath, FileMode::Write);
...
void ImguiMgr::Load_Ui_Desc()
{
   ...
   if (ImGui::Button("Load##Load button", ImVec2(60.f, 20.f)))
   {
      if (!ImGui::IsItemActive())
      {
         ...
         shared_ptr<FileUtils> file = make_shared<FileUtils>();
         file->Open(m_strSaveLoadPath, FileMode::Read);
