...
//LightColor ComputeLight(float3 normal, float2 uv, float3 worldPosition)
//{
//    LightColor color;
//    float4 ambientColor = 0;
//    float4 diffuseRatio = 0;
//    float4 specularRatio = 0;
...
//	// Diffuse
//	{
//        diffuseRatio = saturate(dot(-GlobalLight.direction, normalize(normal)));
//    }
...
//        specularRatio = pow(saturate(dot(-R, E)), 20);
//    }

//	// Emissive
//    emissiveColor = pow(smoothstep(0.0f, 1.0f, 1.0f - saturate(dot(E, normal))), 2);

//    color.diffuse = GlobalLight.diffuse * Material.diffuse * diffuseRatio;
//    color.ambient = GlobalLight.ambient * Material.ambient;
//    color.specular = GlobalLight.specular * specularRatio * Material.specular;
//    color.emissive = GlobalLight.emissive * Material.emissive * e missiveColor;

//    return color;
//}

//void ComputeNormalMapping_Filtered(inout float3 normal, float3 tangent, float2 uv, float2 newUV)
//{
//	// [0,255] 범위에서 [0,1]로 변환
...
//        if (g_bHasMaskMap)
//        {
//            float4 maskColor = MaskMap.Sample(LinearSampler, newUV);
            
//            float4 filterNormal = FilterNormalMap.Sample(LinearSampler, uv);
